...
if(isset($_POST['update_now'])){
   ...
   if($verify_comment->rowCount() > 0){
      $message[] = 'Your comment is already added!';
   }else{
      $update_comment = $conn->prepare("UPDATE `comments` SET comment = ? WHERE id = ?");
      $update_comment->execute([$update_box, $update_id]);
      $message[] = 'Your comment was edited succesfully!';
   }
...
<section class="comments">
   ...
   <div class="show-comments">
      <?php
         ...
         if($select_comments->rowCount() > 0){
            while($fetch_comment = $select_comments->fetch(PDO::FETCH_ASSOC)){
               $select_content = $conn->prepare("SELECT * FROM `CONT` WHERE id = ?");
      ...
      <div class="box" style="<?php if($fetch_comment['user_id'] == $user_id){echo 'order:-1;';} ?>">
         <div class="content"><span><?= $fetch_comment['date']; ?></span><p> - <?= $fetch_content['title']; ?> - </p><a href="COURSES.php?get_id=<?= $fetch_content['id']; ?>">view content</a></div>
      ...
      <?php
         ...
         echo '<p class="empty">There is no comments added!</p>';
      }
      ?>
      </div>
